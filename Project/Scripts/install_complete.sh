...
install_app ()
{
    ...
    git clone https://github.com/MathisHermann/scouti.git
    cd scouti/
...
{  
    ...
    if [[ -f /etc/nginx/sites-enabled/scouti ]]; then
        echo -e "${FONT_BOLD}${FONT_UNDERLINE}nginx config already exists for scouti${FONT_RESET}"
    else
        echo -e "${FONT_BOLD}${FONT_UNDERLINE}Setting up nginx config for scouti${FONT_RESET}"
...
cat > /etc/nginx/sites-available/scouti <<EOF
server {
${tab}listen 80;
${tab}server_name _;
${tab}root /var/www/scouti/public;
...
EOF
        ...
        ln -s /etc/nginx/sites-available/scouti /etc/nginx/sites-enabled/
